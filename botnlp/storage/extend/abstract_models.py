from botnlp.nlu import constants
from botnlp.nlu.conversation import StatementMixin
from botnlp.storage.extend import settings

DJANGO_APP_NAME = constants.DEFAULT_APP_NAME
STATEMENT_MODEL = 'Statement'
TAG_MODEL = 'Tag'

if hasattr(settings, 'CHATNLP'):
    """
    Allow related models to be overridden in the project settings.
    Default to the original settings if one is not defined.
    """
    DJANGO_APP_NAME = settings.CHATNLP.get(
        'django_app_name',
        DJANGO_APP_NAME
    )
    STATEMENT_MODEL = settings.CHATNLP.get(
        'statement_model',
        STATEMENT_MODEL
    )


class AbstractBaseTag(object):
    """
...
class AbstractBaseStatement(object, StatementMixin):
    """
